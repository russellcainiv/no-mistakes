...
When it stops for `ask-user`, it should relay each finding's ID, file, and full description to the user before choosing `approve`, `fix`, or `skip` - except at the review gate itself, where `skip` is not a valid choice: review is mandatory and `--action skip` is rejected there, so `approve` or `fix` are the only options.
